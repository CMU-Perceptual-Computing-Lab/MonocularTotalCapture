...
struct PoseToTransformsHand {
	PoseToTransformsHand(const HandModel &mod, bool euler=true)
		: mod_(mod), euler_(euler) {}
		...
		) const
	{
		...
		for (int idji = 1; idji < mod_.NUM_JOINTS; idji++) {
			...
			if (euler_) ceres::EulerAnglesToRotationMatrix(pose + idj * 3, 3, Rr.data());
			else ceres::AngleAxisToRotationMatrix(pose + idj * 3, Rr.data());
...
struct PoseToTransformsNoLR_Eulers_adamModel {
	PoseToTransformsNoLR_Eulers_adamModel(const TotalModel &mod, const bool euler=true)
		: mod_(mod), euler_(euler)
	{
	}
...
{
public:
	PoseToTransform_AdamFull_withDiff(const TotalModel &mod, const std::array<std::vector<int>, TotalModel::NUM_JOINTS>& parentIndexes, const bool euler=true):
		mod_(mod), mParentIndexes(parentIndexes), euler_(euler)
	{

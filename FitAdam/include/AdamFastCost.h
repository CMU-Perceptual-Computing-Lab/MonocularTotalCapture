...
struct AdamFitData
{
	AdamFitData(const TotalModel &adam, const Eigen::MatrixXd &Joints, const Eigen::MatrixXd &rFoot, const Eigen::MatrixXd &lFoot,
		const Eigen::MatrixXd &faceJoints, const Eigen::MatrixXd &lHandJoints, const Eigen::MatrixXd &rHandJoints, const Eigen::MatrixXd& PAF, const Eigen::MatrixXd& surface_constraint,
		bool fit3D=false, bool fit2D=false, double* K=nullptr, bool fitPAF=false, bool fit_surface=false): 
		...
		surface_constraint(surface_constraint), fit3D(fit3D), fit2D(fit2D), K(K), fitPAF(fitPAF), fit_surface(fit_surface)
		{
			inner_weight.clear();
	...
	const Eigen::MatrixXd& surface_constraint;
	const bool fit3D;
	const bool fit2D;
	const double* K;
	const bool fitPAF;
	const bool fit_surface;
...
{
public:
	AdamFullCost(const AdamFitData& fit_data, const int regressor_type=0, const bool fit_face_exp=false, const bool euler=true):
		fit_data_(fit_data), res_dim(0), start_2d_dim(0), rigid_body(false),
		num_PAF_constraint(fit_data.PAF.cols()), num_inner(fit_data.inner_weight.size()), total_inner_dim(0),
		regressor_type(regressor_type), fit_face_exp(fit_face_exp), euler_(euler)
	{
		if(fit_data_.fit3D)
		{
			res_dim += 3;
		...
		{
			// assert(fit_data_.K);  // Fit a vertex to a specific pixel location.
			assert(fit_data_.surface_constraint.rows() == 6);
		}
        ...
        if(fit_face_exp)
    	{
	        ...
	        dTJdfc_data = new double[3 * TotalModel::NUM_JOINTS * TotalModel::NUM_EXP_BASIS_COEFFICIENTS];
	        std::fill(dTJdfc_data, dTJdfc_data + 3 * TotalModel::NUM_JOINTS * TotalModel::NUM_EXP_BASIS_COEFFICIENTS, 0);  // always 0, output joint not related to face exp
    	}
	}
		...
		if (regressor_type == 0)
		{
			...
			for (int ic = 0; ic < fit_data_.adam.m_indices_jointConst_adamIdx.rows(); ic++)
			{
				...
				if (smcjoint == 4 || smcjoint == 10 || smcjoint == 3 || smcjoint == 9 || smcjoint == 7 || smcjoint == 13)
				{
					// m_targetPts_weight[ic] = BODYJOINT_WEIGHT_Strong;
					// m_targetPts_weight_buffer[ic] = BODYJOINT_WEIGHT_Strong;
					std::fill(m_targetPts_weight.data() + 5 * ic, m_targetPts_weight.data() + 5 * ic + 5, BODYJOINT_WEIGHT_Strong);
					std::fill(m_targetPts_weight_buffer.data() + 5 * ic, m_targetPts_weight_buffer.data() + 5 * ic + 5, BODYJOINT_WEIGHT_Strong);
				}
				else
				{
					// m_targetPts_weight[ic] = BODYJOINT_WEIGHT;
					// m_targetPts_weight_buffer[ic] = BODYJOINT_WEIGHT;
					std::fill(m_targetPts_weight.data() + 5 * ic, m_targetPts_weight.data() + 5 * ic + 5, BODYJOINT_WEIGHT);
					std::fill(m_targetPts_weight_buffer.data() + 5 * ic, m_targetPts_weight_buffer.data() + 5 * ic + 5, BODYJOINT_WEIGHT);
				}
			}
			...
			if (fit_data_.lHandJoints.size() > 0)
			{
				for (int ic = 0; ic < fit_data_.adam.m_correspond_adam2lHand_adamIdx.rows(); ic++)
				{
					// m_targetPts_weight[(ic + offset)] = HANDJOINT_WEIGHT;
					// m_targetPts_weight_buffer[(ic + offset)] = HANDJOINT_WEIGHT;
					std::fill(m_targetPts_weight.data() + 5 * (ic + offset), m_targetPts_weight.data() + 5 * (ic + offset) + 5, HANDJOINT_WEIGHT);
					std::fill(m_targetPts_weight_buffer.data() + 5 * (ic + offset), m_targetPts_weight_buffer.data() + 5 * (ic + offset) + 5, HANDJOINT_WEIGHT);
				}
			...
			for (int i = 0; i < corres_vertex2targetpt.size(); i++)
			{
				...
				std::fill(m_targetPts_weight.data() + 5 * (i + offset), m_targetPts_weight.data() + 5 * (i + offset) + 5, VERTEX_WEIGHT);
				std::fill(m_targetPts_weight_buffer.data() + 5 * (i + offset), m_targetPts_weight_buffer.data() + 5 * (i + offset) + 5, VERTEX_WEIGHT);
			}
		}
		else if (regressor_type == 1)
		{
			for(int i = 0; i < fit_data_.adam.h36m_jointConst_smcIdx.size(); i++)
			{
				...
				std::fill(m_targetPts_weight.data() + 5 * i, m_targetPts_weight.data() + 5 * i + 5, BODYJOINT_WEIGHT);
				std::fill(m_targetPts_weight_buffer.data() + 5 * i, m_targetPts_weight_buffer.data() + 5 * i + 5, BODYJOINT_WEIGHT);
			}
		}
		else
		{
			...
			for (int i = 0; i < 20 * 2; i++)
			{
				...
				std::fill(m_targetPts_weight.data() + 5 * (i + offset), m_targetPts_weight.data() + 5 * (i + offset) + 5, HANDJOINT_WEIGHT);
				std::fill(m_targetPts_weight_buffer.data() + 5 * (i + offset), m_targetPts_weight_buffer.data() + 5 * (i + offset) + 5, HANDJOINT_WEIGHT);
			}
			offset += 40;
			for (int i = 0; i < corres_vertex2targetpt.size(); i++)
			{
	...
	void SparseRegress(const Eigen::SparseMatrix<double>& reg, const double* V_data, const double* dVdP_data, const double* dVdc_data, const double* dVdfc_data,
					   double* J_data, double* dJdP_data, double* dJdc_data, double* dJdfc_data) const;
